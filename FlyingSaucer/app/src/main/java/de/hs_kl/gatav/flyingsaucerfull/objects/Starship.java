...
public class Starship extends Obstacle {

    public boolean enabled=false;
    private boolean active=true;

    private long stopTime;
    private float minTime=10000;
    private float maxTime=30000;
    ...
    public Starship() {
        ...
        setPosition(-20f,0f,20f);  // where no starship has gone before
        setInactive();  // activate counter
        scale = 2.0f;
    ...
    public void draw(GL10 gl) {
        ...
        gl.glRotatef(-90,  0, 1, 0);	// z-direction is front-direction
    ...
    @Override
    // setVelocity to StarShip ensures setting of y-rotation
    ...
    {
        if(!active)
            return active=((System.currentTimeMillis()-stopTime)>timeToWait);
        else
            return false;
    }
    ...
    public void setInactive() {
        if (active) {
            ...
            timeToWait = (maxTime - minTime) * ((float) Math.random())
                    + minTime;
        }
    }
}
